...
public:
	...
	ListNode* BuildHoff(HeapClass* m);			//����Hoffman��
private:
	int TableSize;
	ListNode** TheLists;
};
...
ListNode* HashTbl::BuildHoff(HeapClass* m) {					//����Hoffman��
	int num = 0;
	for (int i = 0; i < this->TableSize; i++) {
		if (this->TheLists[i]->size) {
			m->Insert(this->TheLists[i]);						//������С��
			num++;
		}
	}
	for (int i = 0; i < num - 1; i++) {							//����Hoffman��
...
void MakeHoffman(ListNode* root, int bit) {						//��DFS��Hoffman���е�ÿһ���ַ�����ֵ	
	if (root->Element != "") {									//�����ʵ�Ҷ�ڵ㣬�����ַ�����ֵ����map��
		Hoffman[root->Element] = root->num;
		Hoff << root->Element << " " << root->num << endl;
		return;
	}
	root->Left->num = root->num + 0 * bit;						//�Ը�����ڵ㸳ֵ
	root->Right->num = root->num + 1 * bit;						//�Ը����ҽڵ㸳ֵ
...
int main() {							//O(n)
	...
	ListNode* HoffmanTree = cnt->BuildHoff(t); HoffmanTree->num = 0;//����Hoffman��
	MakeHoffman(HoffmanTree, 1);									//Ϊÿ���ַ�����ֵ
	int len = text.size();
	for (int i = 0; i < len; i++) {									//���ı����б���
		out << Hoffman[text[i]] ;
	}
	return 0;
}
